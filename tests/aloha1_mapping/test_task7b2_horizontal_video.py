    ...
    *,
    ...
    for frame, phase in enumerate(phases):
        ...
        views["overview"]["framing_evidence"] = {
            "visible_prims": list(REQUIRED_FULL_ARM_PRIMS),
            "visible_links": list(REQUIRED_FULL_ARM_LINKS),
        }
        frames.append(
            {
                "physics_frame": frame,
...
def test_manifest_fails_closed_on_missing_framing_or_source_sync(
    ...
    manifest["frames"][4]["views"]["overview"]["framing_evidence"]["visible_links"].remove("elbow")
