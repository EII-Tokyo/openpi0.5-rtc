...
def train_qwen_torch(config: TrainConfig) -> None:
    ...
    model = PiValueQwen3VLTorch(config.model_config, device=device)
    ...
    if config.checkpoint.overwrite and checkpoint_dir.exists() and not config.checkpoint.resume:
    ...
    for step in pbar:
        ...
        grad_norm_value = float(grad_norm.detach().cpu() if isinstance(grad_norm, torch.Tensor) else grad_norm)

        if step % log_every == 0:
            elapsed = time.time() - start_time
            pbar.set_postfix({"loss": f"{loss_value:.4f}", "gn": f"{grad_norm_value:.2f}", "lr": f"{lr:.2e}"})
            if config.logging.wandb_enabled:
                wandb.log(
                    {
                        "loss": loss_value,
        ...
        if (
            ...
            with torch.no_grad():
                val_loss = model.compute_loss(val_batch).mean()
