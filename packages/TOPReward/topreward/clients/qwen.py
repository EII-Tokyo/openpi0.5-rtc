...
class QwenClient(BaseModelClient):
    _PROMPT_MODE_COMPLETION = "completion"
    _PROMPT_MODE_FAILURE_MOMENT = "failure_moment"

    @classmethod
    def _build_instruction_suffix(cls, instruction: str, prompt_mode: str) -> str:
        if prompt_mode == cls._PROMPT_MODE_COMPLETION:
            return (
                f"{instruction} Decide whether the above statement is True or not. "
                "The answer is:"
            )

        if prompt_mode == cls._PROMPT_MODE_FAILURE_MOMENT:
            return (
                f"{instruction}. Answer True only if, at this point in the trajectory, "
                "the task is still achievable without reset and no irreversible failure "
                "has occurred yet. Answer False if an irreversible failure has already "
                "happened (e.g., dropped or lost object, unrecoverable misalignment, "
                "collision or jam, leaving workspace, broken grasp/contact, or any state "
                "from which completion is no longer possible without reset). "
                "The answer is:"
            )

        raise ValueError(
            f"Unknown prompt_mode='{prompt_mode}'. "
            f"Use '{cls._PROMPT_MODE_COMPLETION}' or '{cls._PROMPT_MODE_FAILURE_MOMENT}'."
        )
    ...
    def _load_qwen_model(model_name: str):
        try:
            ...
            if "model type `qwen3_5`" in str(exc):
                raise RuntimeError(
                    "Your transformers build does not support model_type='qwen3_5'. "
                    "Upgrade transformers (the TOPReward repo pins 4.57.1) and retry."
                ) from exc
            raise
        ...
        if model_type == "qwen3_vl":
            ...
            return Qwen3VLForConditionalGeneration.from_pretrained(model_name, **common_kwargs)
        ...
        if model_type == "qwen3_5":
            try:
                ...
                raise RuntimeError(
                    "Model type 'qwen3_5' requires a newer transformers build that exports "
                    "Qwen3_5ForConditionalGeneration. Please upgrade transformers in your runtime."
                ) from exc
            return Qwen3_5ForConditionalGeneration.from_pretrained(model_name, **common_kwargs)

        raise ValueError(f"Unsupported Qwen model_type='{model_type}' for model '{model_name}'.")
    ...
    def __init__(
        ...
        self.model = self._load_qwen_model(model_name)
    ...
    def compute_instruction_reward(
        ...
        instruction_suffix = self._build_instruction_suffix(instruction, prompt_mode)
        ...
        if add_chat_template:
            ...
            full_text = f"{prompt_chat}{instruction_suffix} True"
            image_inputs, video_inputs = process_vision_info(user_messages)  # type: ignore[misc]
