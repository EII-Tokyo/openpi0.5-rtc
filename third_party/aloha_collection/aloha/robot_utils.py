...
def latest_image_qos() -> QoSProfile:
    """Use sensor semantics: retain the newest frame and drop stale work."""
    return QoSProfile(
...
class ImageRecorder:
    def __init__(
        ...
        self.image_qos = latest_image_qos()
        ...
        for cam_name in self.camera_names:
            ...
            self.camera_callback_groups[camera_name] = callback_group
            self.camera_subscriptions[camera_name] = subscription
    ...
    def get_images(self):
        ...
        with self._snapshot_lock:
            return {
                cam_name: getattr(self, f'{cam_name}_image')
                for cam_name in self.camera_names
            }
...
class JointPositionUnwrapper:
    """Keep selected joints continuous across +/-pi wraparound."""

    def __init__(self, joint_indices: Sequence[int] = (3, 5)):
        self.joint_indices = tuple(joint_indices)
        self._last_raw = {}
        self._last_unwrapped = {}

    def unwrap(self, key: str, positions: Sequence[float]) -> np.ndarray:
        raw = np.asarray(positions, dtype=float)
        last_raw = self._last_raw.get(key)
        last_unwrapped = self._last_unwrapped.get(key)
        if last_raw is None or last_unwrapped is None:
            self._last_raw[key] = raw.copy()
            self._last_unwrapped[key] = raw.copy()
            return raw.copy()

        unwrapped = raw.copy()
        for idx in self.joint_indices:
            delta = raw[idx] - last_raw[idx]
            if delta > np.pi:
                delta -= 2 * np.pi
            elif delta < -np.pi:
                delta += 2 * np.pi
            unwrapped[idx] = last_unwrapped[idx] + delta

        self._last_raw[key] = raw.copy()
        self._last_unwrapped[key] = unwrapped.copy()
        return unwrapped
...
def setup_follower_bot(bot: InterbotixManipulatorXS, current_limit=300):
    ...
    set_follower_arm_operating_mode(bot)
...
CONTINUOUS_JOINT_INDICES = tuple(
    JOINT_NAMES.index(name) for name in CONTINUOUS_JOINT_NAMES
)
FOLLOWER_ARM_OPERATING_MODE = 'position'
FOLLOWER_CONTINUOUS_JOINT_OPERATING_MODE = 'ext_position'
START_ARM_POSE = [
...
def set_follower_arm_operating_mode(
    ...
    """Set follower arm modes, optionally enabling multi-turn roll joints."""
    bot.core.robot_set_operating_modes('group', 'arm', FOLLOWER_ARM_OPERATING_MODE)
    if continuous_roll_joints:
        for joint_name in CONTINUOUS_JOINT_NAMES:
            ...
            bot.core.robot_set_operating_modes(
                'single',
                joint_name,
                FOLLOWER_CONTINUOUS_JOINT_OPERATING_MODE,
            )
            try:
                mode = bot.core.robot_get_motor_registers('single', joint_name, 'Operating_Mode')
                print(
                    f"[follower-mode] {bot.core.robot_name}.{joint_name} "
                    f"Operating_Mode={mode} ({FOLLOWER_CONTINUOUS_JOINT_OPERATING_MODE})"
                )
...
FOLLOWER_GRIPPER_JOINT_OPEN = 1.7014
