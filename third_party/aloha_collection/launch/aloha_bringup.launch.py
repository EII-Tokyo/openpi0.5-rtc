    ...
    *,
    ...
    if launch_followers:
        for follower in config.get("follower_arms", []):
            yield "follower", follower


def _run_dynamixel_preflight(context, config):
    if not _launch_bool(context, "dynamixel_preflight"):
        print(
            "[DYNAMIXEL preflight] disabled by launch argument; "
            "no motor readiness check was performed"
        )
        return
    ...
    try:
        ...
        auto_reboot_input_voltage_alerts = _launch_bool(
            context,
        ...
        buses = []
        for role, arm in _selected_arms(
            config,
            launch_leaders=_launch_bool(context, "launch_leaders"),
            launch_followers=_launch_bool(context, "launch_followers"),
        ):
            mode_config_path = PathJoinSubstitution([
                FindPackageShare("aloha"),
                "config",
                f"{role}_modes_{arm['orientation']}.yaml",
            ]).perform(context)
        ...
        raise RuntimeError(
            f"ALOHA bringup aborted before node startup: {error}"
        ) from error
...
def launch_setup(context, *args, **kwargs):
    ...
    _run_dynamixel_preflight(context, config)
...
def generate_launch_description():
    ...
    declared_arguments.append(
        DeclareLaunchArgument(
            ...
            choices=("true", "false"),
            description=(
                "if `true`, ping every configured motor before creating "
                "any launch action"
            ),
        )
    )
    ...
    declared_arguments.append(
        DeclareLaunchArgument(
            "dynamixel_preflight_attempts",
            default_value="3",
            description="maximum ping attempts for each DYNAMIXEL bus",
        )
    )
    ...
    declared_arguments.append(
        DeclareLaunchArgument(
            ...
            choices=("true", "false"),
            description=(
                "if `true`, automatically reboot a motor once when only "
                "a safe historical input-voltage alert is present"
            ),
        )
    )
    ...
    declared_arguments.append(
        DeclareLaunchArgument(
            ...
            default_value="1.0",
            description=(
                "seconds to wait before verifying an automatically "
                "rebooted DYNAMIXEL"
            ),
        )
    )
    ...
    declared_arguments.append(
        DeclareLaunchArgument(
            "dynamixel_preflight_retry_delay",
            default_value="1.0",
            description="seconds between DYNAMIXEL preflight attempts",
        )
    )
