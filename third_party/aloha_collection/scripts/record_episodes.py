...
def opening_ceremony(robots: Dict[str, InterbotixManipulatorXS], gravity_compensation: bool, dt: float) -> None:
    ...
    """
    # Separate leader and follower robots based on naming conventions 翻译：根据命名约定分离领导者和跟随者机器人
    ...
    # Initialize an empty list to store matched pairs of leader and follower robots 翻译：初始化一个空列表来存储匹配的领导者和跟随者机器人对
    pairs = []

    # Create dictionaries mapping suffixes to leader and follower robots 翻译：创建字典映射后缀到领导者和跟随者机器人
    ...
    # Pair leader and follower robots based on matching suffixes 翻译：根据匹配的后缀配对领导者和跟随者机器人
...
def capture_one_episode(
    ...
    for t in tqdm(range(max_timesteps)):
        ...
        # print(f"左：{env.robots['follower_left'].core.robot_get_motor_registers('group', 'arm', 'Operating_Mode')}")

        # print(f"右：{env.robots['follower_right'].core.robot_get_motor_registers('group', 'arm', 'Operating_Mode')}")

        # print(f"爪子：{env.robots['follower_right'].gripper.get_gripper_effort()}")
        # print(f"胳膊：{env.robots['follower_right'].arm.get_joint_efforts()}")


        t1 = time.time() # 获取动作执行时间
        ts = env.step(action) # 执行动作并获取新状态
        t2 = time.time() # 获取环境步长时间
        timesteps.append(ts) # 添加时间步数据
        actions.append(action) # 添加动作数据
        actual_dt_history.append([t0, t1, t2]) # 记录时间历史
        time.sleep(max(0, DT - (time.time() - t0))) # 保持控制频率
    print(f"Avg fps: {max_timesteps / (time.time() - start_time)}") # 打印平均频率

    # End teleoperation and handle torque/gravity settings 结束遥操作并处理扭矩/重力设置
    for name, robot in {name: bot for name, bot in env.robots.items() if "leader" in name}.items():
        if gravity_compensation:
            disable_gravity_compensation(robot) # 禁用重力补偿
        else:
            torque_on(robot) # 开启扭矩 扭矩开启

    # Open grippers on follower robots 打开从动机器人的夹爪
    follower_bots = {name: bot for name,
