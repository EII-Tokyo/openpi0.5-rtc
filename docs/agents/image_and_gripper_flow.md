# Image And Gripper Flow

Read this before changing image capture, HDF5 export, LeRobot conversion, model image transforms, runtime image paths, gripper normalization, or gripper actuation.
...
- Actuation:
  - `/home/eii/openpi0.5-rtc/examples/aloha_real/real_env.py::set_gripper_pose` uses `PUPPET_GRIPPER_JOINT_UNNORMALIZE_FN` then publishes to `/puppet_*` gripper command topics.
