...
## Remote Bottle Bundle Gripper Persistence (2026-08-17)
- The human-accepted free-space full-stroke follower-gripper profile is authored in the reusable override layer `assets/Trossen/ALOHA1/1.0/diagnostics/cad_derived_full_body_colliders/1.0/configuration/aloha1_human_accepted_gripper_control.usda` (SHA-256 `b0b1e9aaca78bef9e22e09dbe03119d680a9a8aa456e74a81ee50f2baaf0e965`). The remote bundle contains the same layer.
- For both followers, the active `left_finger` linear Drive is `type=acceleration`, `maxForce=5 N`, `stiffness=200`, and `damping=50`. The passive `right_finger` remains a PhysX Mimic joint referencing the same follower's `left_finger`, with `naturalFrequency=25 Hz`, `dampingRatio=1`, `gearing=1`, and `offset=0`. Do not add a second Drive to `right_finger`.
- These are human-accepted simulation values, not DYNAMIXEL register values and not a calibrated sim-to-real identification result. Preserve that classification.
- `remote_isaac_assets/aloha1_bottle_server/attempt1/tools/remote_stream_cap_stage_loader.py` is verification-only for this profile. A mismatch must fail startup; do not silently reintroduce session-layer writes. Loader SHA-256: `7048705e516a3ef851b1caf47a422ae04468c9725ec1a9a8f7575742760c6859`.
- Isaac Sim 5.1 Grasp Editor needs a project-archived compatibility patch because `SingleArticulation.prim` can resolve to a root joint that is a sibling of the end-effector link hierarchy. The patch enumerates Gripper Frames from the user's selected gripper root and does not change the robot USD hierarchy. Archive: `remote_isaac_assets/aloha1_bottle_server/attempt1/vendor_patches/isaacsim_grasp_editor_5_1/`.
- The `aloha` server was not restarted when these files were deployed. Offline composition passed on `hxz` and `aloha`; the next natural restart must verify `remote_cap_stage_loader_report.json` contains `runtime_state.gripper_usd_control.left/right` and reports `PASS` before treating startup migration as accepted.
