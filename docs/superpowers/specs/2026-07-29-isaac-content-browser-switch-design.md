...
- Preserve the NVIDIA Assets Full App dependency and both Asset Browser menu
  entries for explicit, on-demand use, per the user's final direction.
...
`omni.kit.browser.asset-1.3.12/config/extension.toml`. Its dependency and lazy
menu trigger were temporarily removed during diagnosis and then restored
...
this design, “switch to Content Browser” means the legacy Isaac Asset Browser
is not selected or started by the Base App dependency graph. NVIDIA Assets
starts as a lightweight menu provider, but it does not create its remote model
or traverse S3 until the user opens the menu.
...
5. Remove only the three legacy Isaac Asset Browser dependency lines with
   deterministic patches; restore the NVIDIA Assets dependency and both menu
   trigger blocks after the user clarifies that manual menu access must remain.
6. Validate all four TOML files and assert that Content Browser remains enabled,
   all three legacy dependencies are absent, the NVIDIA Assets dependency is
   present, and both menu triggers match their original backups.
...
- `omni.kit.browser.asset` starts to register its menu but does not queue remote
  folders before explicit user interaction.
