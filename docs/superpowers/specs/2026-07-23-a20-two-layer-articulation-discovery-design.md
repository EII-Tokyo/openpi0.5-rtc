...
- runtime DOF names and order match Layer 1 and A17 exactly;
- runtime DOF types and limits are finite and consistent with Layer 1 after applying documented Isaac runtime units;
- repeated discovery is deterministic across three fresh process runs.
...
- `PASS_A20_RUNTIME_ARTICULATION_DISCOVERY_NO_STEP`: Layer 2 discovers one deterministic 16-DOF articulation with exact metadata agreement and no prohibited action.
...
Pure helper functions will be test-driven with synthetic expected/observed records, covering exact match, wrong order, duplicate/missing DOFs, invalid limits, wrong units/types, and safety-flag rejection. Layer 1 will then run against the real A17/A19 artifacts. Layer 2 will run through `codex-evidence`; full Isaac logs will stay in `.codex/artifacts/`, while the checked output remains bounded JSON and Markdown.
