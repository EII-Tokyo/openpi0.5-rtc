...
def test_rl_token_query_decoder_shape():
    config = rl_token.RLTokenConfig(
        hidden_dim=8,
        encoder_layers=2,
        decoder_layers=2,
...
def test_rl_token_masked_history_and_margin_loss_shape():
    config = rl_token.RLTokenConfig(
        hidden_dim=8,
        encoder_layers=2,
        decoder_layers=2,
    ...
    loss = model.compute_loss(h_vla, prefix_mask, rng=jax.random.key(1), train=True)
