...
DEFAULT_OUTPUT_DIR = REPO_ROOT / "reports/aloha1_isaac_adaptation/phase108_bottleusd_hdf5_diagnostic_table_gate_20260719"


def _phase108_args(output_dir: Path) -> list[str]:
    return [
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Run the BottleUSD HDF5 drive-target replay with the Phase63 diagnostic fixed-table candidate. "
            "This is not a calibrated final table gate; it verifies that adding the diagnostic support config "
            "does not break the current Phase107 replay and records table/workcell contact provenance."
        )
    )
    parser.add_argument(
        "--python",
    ...
    command = [args.python, *_phase108_args(args.output_dir)]
    if args.dry_run:
