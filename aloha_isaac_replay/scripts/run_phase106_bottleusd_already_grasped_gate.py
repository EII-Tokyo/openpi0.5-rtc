...
DEFAULT_OUTPUT_DIR = REPO_ROOT / "reports/aloha1_isaac_adaptation/phase106_bottleusd_already_grasped_gate_20260719"


def _phase106_args(output_dir: Path) -> list[str]:
    return [
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Run the full-scene Bottle500 already-grasped contact gate. This validates a gripper-relative "
            "BottleUSD pose that keeps bilateral fingertip contact and rejects same-side non-target contacts. "
            "It is not a no-contact-to-grasp active-close gate."
        )
    )
    parser.add_argument(
        "--python",
    ...
    command = [args.python, *_phase106_args(args.output_dir)]
    if args.dry_run:
