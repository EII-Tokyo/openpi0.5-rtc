...
DEFAULT_OUTPUT_DIR = REPO_ROOT / "reports/aloha1_isaac_adaptation/phase107_bottleusd_hdf5_drive_target_gate_20260719"


def _phase107_args(output_dir: Path) -> list[str]:
    return [
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Run the full-scene Bottle500 HDF5 left-arm+gripper drive-target replay gate. "
            "This starts from the Phase106 already-grasped BottleUSD pose and replays a real "
            "ALOHA1 key-region qpos segment under gravity."
        )
    )
    parser.add_argument(
        "--python",
    ...
    command = [args.python, *_phase107_args(args.output_dir)]
    if args.dry_run:
