...
def initialize_runtime_state(stage):
    ...
    for prim in stage.Traverse():
        ...
        if (
            (prim_path == bottle_root or prim_path.startswith(f"{bottle_root}/"))
            and prim.HasAPI(UsdPhysics.RigidBodyAPI)
        ):
            UsdPhysics.RigidBodyAPI(prim).CreateKinematicEnabledAttr(True).Set(True)
    ...
    return {
        ...
        "bottle_kinematic_prim_paths": kinematic_paths,
    }
...
def main() -> int:
    ...
    parser.add_argument(
        "--expected-sha256",
        default="d7da82416f7ac90a681d8fabe74c8402155670ce75b33c7bbbfaa84f4e90b9f8",
    )
    parser.add_argument(
