...
# Project-owned controlled restart for the 103 Isaac Sim streaming server.
# This intentionally does not touch ROS or real-robot services.
...
STAGE="$BUNDLE_ROOT/remote_stream_grasp_stage.usda"
LOADER="$BUNDLE_ROOT/tools/remote_stream_stage_loader.py"
REPORT="$BUNDLE_ROOT/remote_stage_loader_report.json"
LOG="$BUNDLE_ROOT/remote_stream_stage_loader.log"
EXPECTED_SHA=d7da82416f7ac90a681d8fabe74c8402155670ce75b33c7bbbfaa84f4e90b9f8
...
if [ -s "$BUNDLE_ROOT/remote_stream_server.pid" ]; then
  ...
  if [[ "$old_cmd" == *"$APP"* && "$old_cmd" == *"$LOADER"* ]]; then
    kill -TERM "$old_pid"
...
nohup "$KIT" "$APP" \
  ...
  --/app/tokens/omni_documents=/home/eii/isaac-home/Documents \
