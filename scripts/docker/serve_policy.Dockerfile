...
# Build the container:
# docker build . -t openpi_server_low_level -f scripts/docker/serve_policy.Dockerfile

# Run the container:
# docker run --rm -it --network=host -v .:/app --gpus=all openpi_server_low_level /bin/bash
