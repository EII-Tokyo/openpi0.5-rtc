...
class RLTSegmentLedger:
    ...
    def _init_db(self) -> None:
        with self._connect() as conn:
            ...
            self._drop_legacy_quality_columns(conn)
            conn.execute(
                """
    ...
    def _drop_legacy_quality_columns(self, conn) -> None:
        rows = conn.execute("PRAGMA table_info(segments)").fetchall()
        columns = {row[1] for row in rows}
        if columns.issubset(set(SEGMENT_COLUMNS)):
            return
        conn.execute("ALTER TABLE segments RENAME TO segments_legacy_quality")
        conn.execute(
            """
        ...
        shared_columns = [column for column in SEGMENT_COLUMNS if column in columns]
        column_sql = ", ".join(shared_columns)
        conn.execute(
            f"""
            INSERT OR REPLACE INTO segments ({column_sql})
            SELECT {column_sql}
            FROM segments_legacy_quality
            """
        )
        conn.execute("DROP TABLE segments_legacy_quality")
