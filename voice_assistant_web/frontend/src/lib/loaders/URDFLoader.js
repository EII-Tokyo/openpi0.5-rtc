...
class URDFLoader {
    ...
    load(urdf, onComplete, onProgress, onError) {
        ...
        const normalizedUrdf = (urdf || '').trim();
        const workingPath = THREE.LoaderUtils.extractUrlBase(normalizedUrdf);
        const urdfPath = this.manager.resolveURL(normalizedUrdf);
    ...
    defaultMeshLoader(path, manager, done) {
        ...
        // Guard against empty paths before invoking the STL loader.
        if (!path) {
            done(null);
            return;
        }

        // URDF mesh references are simple relative paths, so trimming is sufficient.
        const meshPath = path.trim();
        ...
        loader.load(
            ...
            undefined,
            (error) => {
                console.warn(`URDFLoader: Could not load STL at ${ meshPath }.`, error);
                done(null);
            }
        );
